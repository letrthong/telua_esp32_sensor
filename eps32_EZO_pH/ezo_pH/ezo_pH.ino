...
const char * ssid = "Telua_Ezo_";
const char * ssid_gpio = "Telua_Ezo_Gpio_"; 
...
void initSht4x() {
  Serial.println("Telua pH start the I2C");
...
bool sendReport(bool hasReport) {
  ...
  client -> setInsecure();
  HTTPClient http;
  String serverPath = serverName + "?sensorName=Ezo&pH=" + pH +  "&deviceID=" + deviceID + "&serialNumber=" + serialNumber;

  if(hasGPIo == true){
    serverPath = serverName + "?sensorName=Ezo_Controller&pH=" + pH +  "&deviceID=" + deviceID + "&serialNumber=" + serialNumber;
  }
  
  if (strTriggerParameter.length() > 0) {
    serverPath = trigger_url + "?deviceID=" + deviceID + "&pH=" + pH + "&trigger=" + strTriggerParameter;
  }

  if (hasError == true) {
      serverPath = error_url + "?sensorName=Ezo&deviceID=" + deviceID + "&serialNumber=" + serialNumber;
      if(hasGPIo == true){
          serverPath = error_url + "?sensorName=Ezo_Controller&deviceID=" + deviceID + "&serialNumber=" + serialNumber;
     }
  }
