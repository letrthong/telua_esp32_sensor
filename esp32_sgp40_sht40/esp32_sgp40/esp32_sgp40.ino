...
const char * ssid_gpio = "Telua_SGP4x_Gpio_"; 
...
bool sendReport(bool hasReport) {
 // Serial.println("sendReport");
  ...
  client -> setInsecure();
  HTTPClient http;
  String serverPath = serverName + "?sensorName=SGP4x&voc=" + str_voc_index  + "&temperature=" +strTemp + "&humidity=" +strHumx+   "&deviceID=" + deviceID + "&serialNumber=" + serialNumber;

  if(hasGPIo == true){
    serverPath = serverName + "?sensorName=SGP4x_Controller&voc=" + str_voc_index  +  "&temperature=" +strTemp + "&humidity=" +strHumx + "&deviceID=" + deviceID + "&serialNumber=" + serialNumber;
  }
  
  if (strTriggerParameter.length() > 0) {
    serverPath = trigger_url + "?deviceID=" + deviceID +  "&voc=" + str_voc_index  + "&temperature=" +strTemp + "&humidity=" +strHumx+  +"&trigger=" + strTriggerParameter;
  }

  if (hasError == true) {
      serverPath = error_url + "?sensorName=SGP4x&deviceID=" + deviceID + "&serialNumber=" + serialNumber;
      if(hasGPIo == true){
          serverPath = error_url + "?sensorName=SGP4x_Controller&deviceID=" + deviceID + "&serialNumber=" + serialNumber;
     }
  }
